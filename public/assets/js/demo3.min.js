{
  function debounce(t, i, e) {
    var s;
    return function () {
      var h = this,
        a = arguments,
        n = function () {
          (s = null), e || t.apply(h, a);
        },
        r = e && !s;
      clearTimeout(s), (s = setTimeout(n, i)), r && t.apply(h, a);
    };
  }
  class t {
    constructor(t) {
    ...
    initEvents() {
      this.DOM.nextCtrl.addEventListener("click", () => this.navigate("next")),
        ...
        document.addEventListener("keydown", (t) => {
          ...
          37 === i ? this.navigate("prev") : 39 === i && this.navigate("next");
        });
    }
    navigate(t = "next") {
      if (this.isAnimating) return !1;
      this.isAnimating = !0;
      const i = anime.timeline({
        duration: this.settings.animation.shape.duration,
        easing: this.settings.animation.shape.easing.in,
      });
      i.add({ targets: this.DOM.shape, d: this.paths.final.step1 })
        .add({
          targets: this.DOM.shape,
          d: this.paths.final.step2,
          offset: "-=" + 0.5 * this.settings.animation.shape.duration,
        })
        .add({
          targets: this.DOM.shape,
      ...
      i.finished
        .then(
          () =>
            new Promise((i, e) => {
              const s = this.DOM.slides[this.current];
              anime({
                targets: s,
                duration: this.settings.animation.slides.duration,
                easing: this.settings.animation.slides.easing,
                translateX:
                  "next" === t ? -1 * this.rect.width : this.rect.width,
                complete: () => {
                  s.classList.remove("slide--current"), i();
                },
              }),
                (this.current =
                  "next" === t
                    ...
                    ? this.current - 1
                    : this.slidesTotal - 1);
              const h = this.DOM.slides[this.current];
              h.classList.add("slide--current"),
                anime({
                  targets: h,
                  duration: this.settings.animation.slides.duration,
                  easing: this.settings.animation.slides.easing,
                  translateX: [
                    "next" === t ? this.rect.width : -1 * this.rect.width,
                    0,
                  ],
                });
              const a = h.querySelector(".slide__img");
              anime.remove(a),
                anime({
                  targets: a,
                  duration: 4 * this.settings.animation.slides.duration,
                  easing: this.settings.animation.slides.easing,
                  translateX: ["next" === t ? 200 : -200, 0],
                }),
                anime({
                  targets: [
                    h.querySelector(".slide__title"),
                    h.querySelector(".slide__desc"),
                    h.querySelector(".slide__link"),
                  ],
                  duration: 2 * this.settings.animation.slides.duration,
                  easing: this.settings.animation.slides.easing,
                  delay: (t, i) => 100 * i + 100,
                  translateX: ["next" === t ? 300 : -300, 0],
                  opacity: [0, 1],
                });
            })
        )
        .then(() => {
          anime
            ...
            .add({ targets: this.DOM.shape, d: this.paths.final.step3 })
            .add({
              targets: this.DOM.shape,
              d: this.paths.final.step2,
              offset: "-=" + 0.5 * this.settings.animation.shape.duration,
  ...
  new t(document.querySelector(".slideshow")),
